...
import { triggerNotionAuthFlow } from './popup/util/auth';
...
export default defineBackground(
	...
	async function initialize() {
		...
		if (!info || !('userId' in info)) {
			console.log(
				'User not logged into Halo. Extension will work once user logs in.'
			);
			return;
		}
		...
		try {
			const userInfo = await getHaloUserInfo({
				cookie: cookies
			});
			if (userInfo) {
	...
	// FIREFOX RESTRICTION: popup is closed during auth, so it needs to be triggered from background script
		chrome.runtime.onMessage.addListener(
			(msg: any, sender: any, sendResponse: any) => {
				...
				else if (msg === 'launch_auth') {
					(async () => {
						try {
							await triggerNotionAuthFlow();
							sendResponse(null);
						} catch (error) {
