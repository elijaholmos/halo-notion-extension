...
class ChromeStorageSyncStore {
	...
	private constructor({ key, initial_value }: { key: string; initial_value: any }) {
		this.key = key;
	...
	static async create({ key, initial_value }: { key: string; initial_value: any }) {
		const instance = new ChromeStorageSyncStore({ key, initial_value });
		const item = await chrome.storage.sync.get(key);
		console.log(`in callback for ${key}`, initial_value, item);
		initial_value === null && !!Object.keys(item).length ? instance.set(item[key]) : instance.set(initial_value);
		return instance;
	}
	...
	update(new_value: any) {
		const isLiteralObject = (o: any) => !!o && o.constructor === Object; //https://stackoverflow.com/a/16608074/8396479
		const { value } = this;
...
export default async function ({ key, initial_value = null }: { key: string; initial_value: any }) {
	initial_value instanceof Function && (initial_value = await initial_value());
	const custom_store = await ChromeStorageSyncStore.create({ key, initial_value });
	let {
		store: { subscribe },
		get,
		set,
