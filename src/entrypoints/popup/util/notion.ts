...
// screw CORS.
const proxy = async function (url: string, options: any) {
	return (
		(await (
...
export const getAssignmentsDatabaseId = async function () {
	...
	return (database_id = results?.find((r: any) => 'Course' in r.properties)?.id);
};

export const createPage = async function ({ database_id, ...data }: { database_id: string; [key: string]: any }) {
...
const chunkSubstr = function (str: string, size: number) {
...
export const importSingleAssignment = async function ({ assessment, current_class, unit, totalpoints }: { assessment: any, current_class: any, unit: any, totalpoints: number }) {
	...
	return await createPage({
		...
		properties: {
			...
			URL: {
				type: 'url',
				url: parseURL(assessment, current_class.slugId as string),
			},
			...
			'Start date': {
				type: 'date',
				date: {
					start: formatISO(parseISO(assessment.startDate)),
				},
			},
			'Due date': {
				type: 'date',
				date: {
					start: formatISO(parseISO(assessment.dueDate)),
				},
			},
			Tags: {
				type: 'multi_select',
			...
			Topic: {
				...
				multi_select: [
					{
						name: unit.title.split(':')[0] as string,
					},
				],
			},
			Status: {
		...
		children: [
			...
			...(!!assessment.attachments.length
				? assessment.attachments.map((att: any) => ({
						object: 'block',
						type: 'file',
						file: {
							caption: [
...
export const populateClassAssignments = async function (slugId: string) {
	...
	const { ['userId']: _, ...remainderCookie } = await getInformation() as any;
	const current_class = await getClassInformation({ cookie: remainderCookie, slugId: slugId as string });
